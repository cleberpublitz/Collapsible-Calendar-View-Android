package com.github.cleberpublitz.collapsiblecalendarview.view
...
class ExpandIconView @JvmOverloads constructor(context: Context, attrs: AttributeSet? = null, defStyleAttr: Int = 0) : View(context, attrs, defStyleAttr) {
    ...
    private var alphaIcon = MORE_STATE_ALPHA
    ...
    init {
        ...
        val array = getContext().theme.obtainStyledAttributes(attrs, R.styleable.ExpandIconView,
                0, 0)
        ...
        try {
            ...
            color = array.getColor(R.styleable.ExpandIconView_eiv_color, BLACK)
            colorMore = array.getColor(R.styleable.ExpandIconView_eiv_colorMore, BLACK)
            colorLess = array.getColor(R.styleable.ExpandIconView_eiv_colorLess, BLACK)
    ...
    private fun updateArrowPath() {
        ...
        if (left != null && right != null) {
            rotate(left, (-alphaIcon).toDouble(), tempLeft)
            rotate(right, alphaIcon.toDouble(), tempRight)
    ...
    private fun animateArrow(toAlpha: Float) {
        ...
        val valueAnimator = ValueAnimator.ofFloat(alphaIcon, toAlpha)
        valueAnimator.addUpdateListener(object : ValueAnimator.AnimatorUpdateListener {
            private val colorEvaluator = ArgbEvaluator()

            override fun onAnimationUpdate(valueAnimator: ValueAnimator) {
                alphaIcon = valueAnimator.animatedValue as Float
                updateArrowPath()
                if (switchColor) {
    ...
    private fun updateColor(colorEvaluator: ArgbEvaluator) {
        ...
        if (colorIntermediate != -1) {
            colorFrom = if (alphaIcon <= 0f) colorMore else colorIntermediate
            colorTo = if (alphaIcon <= 0f) colorIntermediate else colorLess
            fraction = if (alphaIcon <= 0) 1 + alphaIcon / 45f else alphaIcon / 45f
        } else {
            colorFrom = colorMore
            colorTo = colorLess
            fraction = (alphaIcon + 45f) / 90f
        }
    ...
    private fun calculateAnimationDuration(toAlpha: Float): Long {
        return (Math.abs(toAlpha - alphaIcon) / animationSpeed).toLong()
    }

    private fun rotate(startPosition: Point, degrees: Double, target: Point) {
        val angle = Math.toRadians(degrees)
        val x = (center.x + (startPosition.x - center.x) * Math.cos(angle) - (startPosition.y - center.y) * Math.sin(angle)).toInt()

        val y = (center.y.toDouble() + (startPosition.x - center.x) * Math.sin(angle) +
                (startPosition.y - center.y) * Math.cos(angle)).toInt()

        target.set(x, y)
    }

    private fun postInvalidateOnAnimationCompat() = postInvalidateOnAnimation()
